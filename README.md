...
Note: Algorithm 4 also covers the invalid input case (negative number) — when n <= 0, the else branch prints "Invalid! Please try again" and the factorial loop is skipped entirely.
...
Algorithm 9: Check if a String is Palindrome (Two-Pointer Method)
...
Note: Algorithm 12 also covers the "Element Not Found" case — when the key does not exist in the list, the while loop completes without executing break, so the else block runs and prints "Element not found".
